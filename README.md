...
Reflective draft reviews are written as JSON by the `review-draft-report` command (see `docs/text-review-guide.md` and `docs/review-report-schema.md`). Each report has a short summary plus five feedback cards (title, clarity, logic, voice, emotional). Each card has a stage line (Strong / Good enough / Needs attention), a **What's good** block, and a **What needs attention** block when something substantive applies.
